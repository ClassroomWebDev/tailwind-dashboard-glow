...
/** Standalone entry form page — rendered at the dedicated /sales/new route. */
export function NewOpportunityPage({ preselect }: { preselect?: string | undefined }) {
  return (
    <div className="space-y-8">
      <header>
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Opportunity</p>
        <h1 className="mt-1 font-display text-3xl font-bold tracking-tight">New opportunity</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Enrol a student at the student special price — leadership points arrive once the opportunity is approved.
        </p>
      </header>

      <OpportunityEntry preselect={preselect} />
      <OpportunityHistory />
    </div>
  );
}
...
function OpportunityEntry({ preselect }: { preselect?: string | undefined }) {
  const { data: role } = useMyRole();
  ...
  const [selection, setSelection] = useState(preselect ?? "");
  ...
  // Read-only preview pills for the selected course / programme.
  ...
  return (
    <section className="rounded-3xl border border-border bg-card p-6 shadow-sm sm:p-8">
      ...
      <div className="mt-6 grid gap-5 sm:grid-cols-2">
        ...
        {preview ? (
          <div className="flex flex-wrap gap-2 sm:col-span-2">
            ...
            <span className="rounded-md bg-red-800 px-3 py-1 text-xs font-bold text-white">
              Student price: {money(preview.student)}
            </span>
            {preview.learning > 0 ? (
              <Badge variant="outline">+{preview.learning} Learning Points / class</Badge>
            ) : null}
        ...
        <div className="grid gap-1.5">
          ...
          <Input
            ...
            placeholder="Transaction ID / account no"
          />
        </div>
        <div className="grid gap-1.5 sm:col-span-2">
          <Label className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
      ...
      <Button className="mt-6" onClick={() => void submit()} disabled={saving}>
        {saving ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />} Record opportunity
      </Button>
    </section>
  );
}
